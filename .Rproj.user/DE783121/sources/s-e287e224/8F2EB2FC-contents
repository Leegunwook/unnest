...
# 특수문자를 공백으로 치환 및 애피소드 컬럼 적용 (형태소 분석)
...
######################## f_s_token 데이터 내 에피소드 별 단어 카운트
total_words <- f_s_token %>%
  group_by(episode) %>%
  summarize(total = sum(n))

####################### 에피소드 번호 기준 단어의 전체 개수와 total_words를 left_join
f_s_token <- left_join(f_s_token, total_words)
#view(episode_words)

######################df_idf 함수를 사용하여, 각각의 단어가 시나리오(에피소드) 내 tf-idf 산출
f_s_token <- f_s_token %>% 
  bind_tf_idf(word, episode, n)

##################### 가중치(tf-idf) 높은 순서로 내림 차순 정렬
f_s_token %>% 
  arrange(desc(tf_idf))

####################전체 단어 중 가중치 상위 20개 단어 시각화
f_s_token %>% 
  arrange(desc(tf_idf)) %>% 
...
#################### 시나리오(에피소드)별 가중치 상위 10개 단어에 대한 시각화
f_s_token %>% 
  arrange(desc(tf_idf)) %>% 
  group_by(episode) %>% 
  top_n(10) %>% 
